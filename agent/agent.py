...
class Agent:
    ...
    def think(self, context: dict) -> dict:
        ...
        # On first turn (no intro set yet) or clear-memory request
        if context.get("clear_memory", False) or len(self.messages) == 0:
            intro = context.get("intro", "")
            if not intro:
                intro = context.get("prompt", "")
            if intro:
                self.set_intro(intro)

        # Build API messages: persistent history + current situational context
        api_messages = list(self.messages)
        recent = context.get("recent_context", "")
        if recent:
            api_messages.append({"role": "user", "content": recent})
